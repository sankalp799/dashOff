...
function timeStamp() { if (window.performance.now) { return window.performance.now() } else { return Date.now() } }

function isVisible(el) { var r = el.getBoundingClientRect(); return r.top + r.height >= 0 && r.left + r.width >= 0 && r.bottom - r.height <= (window.innerHeight || document.documentElement.clientHeight) && r.right - r.width <= (window.innerWidth || document.documentElement.clientWidth) }
...
function WarpSpeed(targetId, config) {
    ...
    this.prevH = -1;
    this.stars = [];
    for (var i = 0; i < this.DENSITY * 1000; i += 1) { this.stars.push(new Star((Math.random() - 0.5) * 1000, (Math.random() - 0.5) * 1000, 1000 * Math.random())) }
...
WarpSpeed.prototype = {
        ...
        draw: function() {
            var TIME = timeStamp();
            if (!(document.getElementById(this.targetId))) { this.destroy(); return }
            this.move();
            var canvas = document.getElementById(this.targetId);
            if (!this.PAUSED && isVisible(canvas)) {
                if (this.prevW != canvas.clientWidth || this.prevH != canvas.clientHeight) {
                    ...
                    canvas.height = (canvas.clientHeight < 10 ? 10 : canvas.clientHeight) * (window.devicePixelRatio || 1) ctx.fillStyle = this.STAR_COLOR;
                }
                ...
                for (var i = 0; i < this.stars.length; i += 1) {
                    ...
                    if (this.WARP_EFFECT) {
                        ...
                        ctx.fill()
                    } else { ctx.fillRect(canvas.width * (xOnDisplay + 0.5) - size / 2, canvas.height * (yOnDisplay + 0.5) - size / 2, size, size) }
                }
        ...
        move: function() {
            ...
            function WarpSpeed(targetId, config) {
                ...
                this.WARP_EFFECT_LENGTH = config.warpEffectLength == undefined ? 5 : config.warpEffectLength < 0 ? 0 : config.warpEffectLength;
                this.STAR_SCALE = config.starSize == undefined || config.starSize <= 0 ? 3 : config.starSize;
                this.BACKGROUND_COLOR = config.backgroundColor == undefined ? "hsl(263,45%,7%)window.requestAnimationFrame=window.requestAnimationFrame||(function(callback,element){setTimeout(callback,1000/60)});function timeStamp(){if(window.performance.now){return window.performance.now()}else{return Date.now()}}function isVisible(el){var r=el.getBoundingClientRect();return r.top+r.height>=0&&r.left+r.width>=0&&r.bottom-r.height<=(window.innerHeight||document.documentElement.clientHeight)&&r.right-r.width<=(window.innerWidth||document.documentElement.clientWidth)}function Star(x,y,z){this.x=x;this.y=y;this.z=z;this.size=0.5+Math.random()}function WarpSpeed(targetId,config){this.targetId=targetId;if(WarpSpeed.RUNNING_INSTANCES==undefined){WarpSpeed.RUNNING_INSTANCES={}}if(WarpSpeed.RUNNING_INSTANCES[targetId]){WarpSpeed.RUNNING_INSTANCES[targetId].destroy()}config=config||{};if(typeof config=="
                ...
                if (targetId) { if (WarpSpeed.RUNNING_INSTANCES[targetId]) { WarpSpeed.RUNNING_INSTANCES[targetId].destroy() } } else {
                    try { cancelAnimationFrame(this.drawRequest) } catch (e) { this.drawRequest = -1 }
                    WarpSpeed.RUNNING_INSTANCES[this.targetId] = undefined
                }
            }, pause: function() { this.PAUSED = true }, resume: function() { this.PAUSED = false }
        };
