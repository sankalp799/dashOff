...
/***********
dev.url_shortener('dashoff.herokuapp.com/join?id=$9iqkxpn4ckg', (err, url) => {
    console.log(err, url);
});
**/
...
dev.fetch_images = async (word, callback) => {
    let url = `https://api.unsplash.com/search/photos?query=${word}&client_id=${process.env.IMAGE_API_CLIENT_ID}`;
