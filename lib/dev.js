...
dev.fetchPlayerScore = (rooms, user, callback) => {
    ...
    if (playerlist !== null) {
        ...
        let playerslistmessage = dev.createMessage(typeMessage.SCORE, user, sortedList);
...
dev.wordList = () => {
    let div = categories.length;
    div = Math.floor(div / 3);
    let words = [];
    for (let i = 0; i < 3; i++) {
        // push three random and unique words from WORDS_LIST
        words.push(categories[(Math.floor(Math.random() * (div))) + (div * i)]);
    }
    return words;
};
