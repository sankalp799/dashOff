...
let mainServer = (req, res) => {
    ...
    if(urlPath.includes('music/')){
        const mid = urlPath.split('/')[1];
        const y_url = `https://www.youtube.com/watch?v=${mid}`
        console.log(y_url);
        ytdl.getInfo(y_url)
            .then(data => {
...
Server.httpsServer = https.createServer(options, (req, res) => {
    mainServer(req, res);
})
...
Server._redi_t_rid_pur_ = (id, uname, callback) => {
    console.log(id);
    let rindex = rooms.findIndex(r => r.roomId == id);
    console.log(rindex);
    if(rindex >= 0){
        let link = `${_config.URL.PROTOCOL}://${_config.URL.ORIGIN}/id=${id}`;
        callback(200, {link}, 'json'); 
    }else{
        callback(500, {'Error': 'could not find any public room currently please try again later'}, 'json');
    }
}
...
Server.goTo_public_toilet = (data, callback) => {
    ...
    try{
        let rid = false;
        let prs = rooms.filter(r => r.rType.toLowerCase() == 'public');
        if(prs.length > 0){
            let lp_pr = prs.filter(pu_rm => pu_rm.players.length <= 2);
            rid = lp_pr.length > 0 ? lp_pr : prs;
            rid = rid[Math.floor(Math.random() * rid.length)].roomId;
            // send user to room with rid
            Server._redi_t_rid_pur_(rid, data.payload['username'], callback);
        }else
            callback(404, {'Error': 'No Public Rooms found. Host one by yourself'}, 'json');
    }catch(e){
        console.log('error> ', e);
        callback(500, {'Error': 'could not find any public room currently please try again later'}, 'json');
    }
...
Server.router = {
    ...
    '.well-known/pki-validation/60964F2A333AB3A669BCC1DCB10DA7E0.txt': _handler.free_ssl
};
...
Server.init = () => {
    Server.httpServer.listen(PORT, (error) => !error ? console.log(`listening on port ${PORT}`) : console.log(`error> ${error.message}`));    
};
