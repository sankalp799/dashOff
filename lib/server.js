...
WebSocketServer.on('connection', (socket, request) => {
    ...
    rooms.forEach((element) => {
        if (element.roomId == data.id) {
            ...
            // console.log('room index ' + joined_room_index);
            if (!element.playing) {
                joined = element.addPlayer(request.socket.remotePort, data.username);
                // console.log('joined with data: ', joined);
                if (joined) {
...
Server.createRoom = (data, callback) => {
    console.log(data.payload.username, data.payload.rounds);
    if (data.method.toLowerCase() == 'post') {
        let game_rounds = parseInt(data.payload.rounds);
        let round_counter = parseInt(data.payload.counter);
        let hostUserName = typeof(data.payload.username) == 'string' && data.payload.username.trim().length > 0 && data.payload.username !== 'null' && data.payload.username !== null ? data.payload.username.trim() : false;
        ...
        if (hostUserName && round_counter) {
            ...
            if (roomId) {
                let newRoom = new _room(roomId, hostUserName, round_counter, game_rounds, HOST, PORT, _config.URL.PROTOCOL || 'http');
                ...
                rooms.push(newRoom);
                // console.log(newRoom);
                callback(200, { 'link': newRoom.link }, 'json');
            } else {
                _handler.error({ header: '', error: 'could not create room please try again' }, callback);
            }
        } else {
            callback(400, { 'Error': 'Invalid Username' }, 'json');
        }
    } else {
