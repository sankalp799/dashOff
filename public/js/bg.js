const bg = document.getElementById('main-bg-canvas');
...
window.addEventListener('resize', (e) => {
    bg.width = html_Dom.offsetWidth;
    bg.height = html_Dom.offsetHeight;
    console.log(html_Dom.offsetWidth, bg.width);
});
...
class Particle {
    constructor(width, height) {
        ...
        this.baseY = this.y;
        this.speedX = Math.random() < 0.5 ? -0.50 : 0.5;
        this.speedY = Math.random() < 0.5 ? -0.50 : 0.50;
        this.density = Math.random() * 50;
        this.displacedX = false;
        this.displacedY = false;
    }
    ...
    update() {
        ...
        if (dis < mousePos.d) {
            this.x -= Fx * 5;
            this.y -= Fy * 5;
            this.displacedX = true;
            this.displacedY = true;
        } else if (this.displacedX || this.displacedY) {
